...
- Payments are real, through Moyasar. `POST /api/orders` reserves a PENDING order and returns a hosted invoice URL; entitlements are granted only by `complete_paid_order`, which runs after the API re-reads the payment from Moyasar and confirms the charged amount matches the order. Never grant an entitlement from anything the browser sends, and never take an amount from a request body. Free checkout exists only when `PAYMENTS_MODE=demo` is set explicitly — a missing key must break checkout, not give books away.
